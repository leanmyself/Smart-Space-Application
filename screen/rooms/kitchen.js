...
const Item = ({ data }) => {
  ...
  useEffect(() => {
    const unsubscribe = firestore().collection('home-devices').doc('KITCHEN').onSnapshot((documentSnapshot) => {
      if (data.title === "Fan") {
        toggleSwitch(documentSnapshot.data().FAN === "FKHIGH");
      } else if (data.title === "Light Bulb") {
        toggleSwitch(documentSnapshot.data().LIGHTS === "LKHIGH");
      }
      else if (data.title === "Outlet") {
        toggleSwitch(documentSnapshot.data().OUTLET === "OKHIGH");
      }
    });
    return () => {
      unsubscribe();
  ...
  const handleSwitchChange = async () => {
    ...
    const fanSwitchValue = newSwitchValue ? "FKHIGH" : "FKLOW";
    const lightSwitchValue = newSwitchValue ? "LKHIGH" : "LKLOW";
    const outletSwitchValue = newSwitchValue ? "OKHIGH" : "OKLOW";
    const batch = firestore().batch();
    const docRef = firestore().collection('home-devices').doc('KITCHEN');
