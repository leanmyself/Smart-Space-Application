...
const Item = ({ data }) => {
  ...
  useEffect(() => {
    const unsubscribe = firestore().collection('home-devices').doc('BEDROOM').onSnapshot((documentSnapshot) => {
      if (data.title === "Fan") {
        toggleSwitch(documentSnapshot.data().FAN === "FBRHIGH");
      } else if (data.title === "Light Bulb") {
        toggleSwitch(documentSnapshot.data().LIGHTS === "LBRHIGH");
      } else if (data.title === "Outlet") {
        toggleSwitch(documentSnapshot.data().OUTLET === "OBRHIGH");
      }
    });
    return () => {
      unsubscribe();
  ...
  const handleSwitchChange = async () => {
    ...
    const fanSwitchValue = newSwitchValue ? "FBRHIGH" : "FBRLOW";
    const lightSwitchValue = newSwitchValue ? "LBRHIGH" : "LBRLOW";
    const outletSwitchValue = newSwitchValue ? "OBRHIGH" : "OBRLOW";
    const batch = firestore().batch();
    const docRef = firestore().collection('home-devices').doc('BEDROOM');
